...
public readonly struct RGBSpectrum : ISpectrum, IEquatable<RGBSpectrum> {
    ...
    public readonly float Green => rgb.Y;
    /// <summary> The blue component of the <see cref="RGBSpectrum"/> </summary>
    public readonly float Blue => rgb.Z;
    ...
    public int ToRGBInt() {
        var color = Vector3.Clamp(rgb, Vector3.Zero, Vector3.One);
        var r = ((int)(color.X * 255f)) << 16;
        var g = ((int)(color.Y * 255f)) << 8;
        var b = ((int)(color.Z * 255f)) << 0;
        return r + g + b;
    }

    ...
    public class BVH : AccelerationStructure, IBinaryTree {
        ...
        Split? BestSweepSplit() {
            List<Split?> splits = new() {
                BestLinearSplitAfterSort(Vector3.UnitX, p => p.BoundingBox.Center.X),
                BestLinearSplitAfterSort(Vector3.UnitY, p => p.BoundingBox.Center.Y),
                BestLinearSplitAfterSort(Vector3.UnitZ, p => p.BoundingBox.Center.Z)
            };
        ...
        Split? BestLinearSplitAfterSort(Vector3 sortDirection, Func<ISceneObject, float> axisSortingFunc) {
        ...
        Split? BestBinSplit(Vector3 binningDirection, Func<Vector3, float> axis) {
            float k1 = BinAmount * BinningEpsilon / axis(BoundingBox.Size);
            float bestSplitCost = SurfaceAreaHeuristic(Items.Count, BoundingBox.SurfaceArea);
            ...
            for (int i = 1; i < BinAmount; i++) {
                ...
                foreach (ISceneObject primitive in Items) {
                    int binID = (int)(k1 * (axis(primitive.BoundingBox.Center) - axis(BoundingBox.MinCorner)));

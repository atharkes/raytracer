...
public class Renderer : IRenderer {
    ...
    public void Render(TimeSpan renderTime) {
        ...
        // Integration
        var integrationTime = renderTime - Statistics.OpenTKTime.LastTick - Statistics.DrawingTime.LastTick;
        var sampleCount = IntegrationSampleCount(integrationTime);
    ...
    private int IntegrationSampleCount(TimeSpan integrationTime) {
        var sampleCount = 0;
        if (Statistics.IntegratorTime.LastTick.Ticks > 0) {
            var previousTime = Statistics.IntegratorTime.LastTick;
            var sampleCountFactor = 1 + (integrationTime / previousTime - 1) * 0.1;

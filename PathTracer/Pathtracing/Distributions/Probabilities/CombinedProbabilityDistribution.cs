...
public class CombinedProbabilityDistribution<T> : IProbabilityDistribution<T>, IEquatable<CombinedProbabilityDistribution<T>> where T : notnull {
    ...
    private readonly HashSet<IProbabilityDistribution<T>> items = new();
    private readonly Dictionary<IProbabilityDistribution<T>, double> weights = new();
    private readonly Dictionary<IProbabilityDistribution<T>, double> probabilities = new();
    private readonly Dictionary<IProbabilityDistribution<T>, double> cummulativeProbabilities = new();
    ...
    public CombinedProbabilityDistribution(params (IProbabilityDistribution<T> ProbabilityDistribution, double Weight)[] pairs) {
        ...
        foreach ((var distribution, var weight) in pairs) {
            if (items.Contains(distribution)) {

    ...
    public class Union : IShape, ICollection<IShape> {
        ...
        public IEnumerable<Position1> IntersectDistances(IRay ray) {
            foreach (IShape shape in Shapes) {
                foreach (Position1 distance in shape.IntersectDistances(ray)) {
        ...
        public Normal3 OutwardsDirection(Position3 position) {
            throw new NotImplementedException("Requires to find the shape with the surface closest to the position");
        }
        ...
        public Position3 SurfacePosition(Random random) {
            throw new NotImplementedException("Sample based on surface area?");
        }

        public Position2 UVPosition(Position3 position) {
            throw new NotImplementedException("Requires combining of UV-maps");
        }
    }
}

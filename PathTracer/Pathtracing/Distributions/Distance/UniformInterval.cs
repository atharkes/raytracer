...
public readonly struct UniformInterval : IDistanceDistribution {
    ...
    public double MaterialDensity(Position1 distance) => Domain.Includes(distance) ? 1d / (Domain.Exit - distance) : 0f;

    public Position1 Sample(Random random) => Domain.Entry + (float)(random.NextDouble() * Domain.CoveredArea);

    public double ProbabilityDensity(Position1 sample) => Domain.Includes(sample) ? 1d / Domain.CoveredArea : 0d;

    public double CumulativeProbability(Position1 sample) => Math.Min(Math.Max(0d, ((float)sample).Previous() - Domain.Entry), Domain.CoveredArea) / Domain.CoveredArea;

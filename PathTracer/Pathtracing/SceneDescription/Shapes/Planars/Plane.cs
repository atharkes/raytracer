...
public readonly struct Plane : IPlanarShape {
    ...
    public float DistanceToSurface(Position3 position) => Math.Abs(Direction3.Dot(position - Position, Normal));
    ...
    public Position3 Project(Position3 position) => position + Normal * (float)Direction3.Dot(Position - position, Normal);
    ...
    public Position1? IntersectDistance(IRay ray) => -((Vector3.Dot(ray.Origin.Vector, Normal.Vector) - Distance.Vector) / Vector3.Dot(ray.Direction.Vector, Normal.Vector));

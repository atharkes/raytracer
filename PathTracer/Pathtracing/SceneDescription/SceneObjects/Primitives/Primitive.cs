...
public class Primitive : IPrimitive {
    ...
    public IDistanceQuery? Trace(IRay ray, ISpectrum spectrum) {
        var intervals = Shape.Intersect(ray);
        ...
        foreach (var interval in intervals) {
            var distanceDistribution = Material.DensityProfile.GetDistances(ray, spectrum, interval);
    ...
    public virtual IEnumerable<ISceneObject> Clip(AxisAlignedPlane plane) {
        foreach (var shape in Shape.Clip(plane)) {
            yield return shape == Shape ? this : (ISceneObject)new PrimitiveFragment(this, shape);
        }
    }
}

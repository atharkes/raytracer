...
public readonly struct SurfaceHalfEllipsoid : IDirectionDistribution, IEquatable<SurfaceHalfEllipsoid> {
    ...
    private Vector3 SampleVNDF(float r1, float r2) {
        ...
        var up = Normal3.UnitZ;
        var rotation = Orientation == -up
            ? new OpenTK.Mathematics.Quaternion(Normal3.AnyPerpendicular(up).Vector, 0f)
            : new OpenTK.Mathematics.Quaternion(Vector3.Cross(Orientation.Vector, up.Vector), Vector3.Dot(Orientation.Vector, up.Vector) + 1f).Normalized();
        var Ve = rotation * -IncomingDirection.Vector;

        // Section 3.2: transforming the view direction to the hemisphere configuration
        var Vh = new Vector3(Roughness * Ve.X, Roughness * Ve.Y, Ve.Z).Normalized();

        // Section 4.1: orthonormal basis (with special case if cross product is zero)
        var lensq = Vh.X * Vh.X + Vh.Y * Vh.Y;
        var T1 = lensq > 0 ? new Vector3(-Vh.Y, Vh.X, 0) / (float)Math.Sqrt(lensq) : new Vector3(1f, 0f, 0f);
        var T2 = Vector3.Cross(Vh, T1);

        // Section 4.2: parameterization of the projected area
        var r = (float)Math.Sqrt(r1);
        var phi = 2.0f * (float)Math.PI * r2;
        var t1 = r * (float)Math.Cos(phi);
        var t2 = r * (float)Math.Sin(phi);
        var s = 0.5f * (1f + Vh.Z);
        t2 = (1f - s) * (float)Math.Sqrt(1f - t1 * t1) + s * t2;

        // Section 4.3: reprojection onto hemisphere
        var Nh = t1 * T1 + t2 * T2 + (float)Math.Sqrt(Math.Max(0f, 1f - t1 * t1 - t2 * t2)) * Vh;

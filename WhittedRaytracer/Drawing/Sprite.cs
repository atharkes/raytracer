...
namespace WhittedRaytracer.Drawing {
    ...
    public class Sprite {
        ...
        public void Draw(float x, float y, float scale = 1.0f) {
            ...
            float u1 = (x * 2 - 0.5f * scale * bitmap.Width) / OpenTKProgram.GameWindow.Width - 1;
            float v1 = 1 - (y * 2 - 0.5f * scale * bitmap.Height) / OpenTKProgram.GameWindow.Height;
            float u2 = ((x + 0.5f * scale * bitmap.Width) * 2) / OpenTKProgram.GameWindow.Width - 1;
            float v2 = 1 - ((y + 0.5f * scale * bitmap.Height) * 2) / OpenTKProgram.GameWindow.Height;

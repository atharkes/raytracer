...
public class PinholeCamera : ICamera {
    ...
    public void SetViewDirection(Normal3 direction) => Rotate(new Quaternion(Vector3.Cross(ViewDirection.Vector, direction.Vector), Vector3.Dot(ViewDirection.Vector, direction.Vector) + 1).Normalized());
    ...
    public CameraRay GetCameraRay(Position2 position, Direction2 direction) {
        var rayOrigin = Film.Rectangle.SurfacePosition(position);

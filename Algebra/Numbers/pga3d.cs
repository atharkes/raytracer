...
public class PGA3D {
    ...
    public readonly Number[] _mVec = new Number[16];
    ...
    public PGA3D(Number f = 0f, byte idx = 0) {
        _mVec[idx] = f;
    }

    public PGA3D(Number[] array) {
    ...
    /// </summary>
    public static PGA3D operator *(Number a, PGA3D b) {
        var result = new PGA3D();
        result[0] = a * b[0];
        result[1] = a * b[1];
        result[2] = a * b[2];
    ...
    /// </summary>
    public static PGA3D operator *(PGA3D a, Number b) {
        var result = new PGA3D();
        result[0] = a[0] * b;
        result[1] = a[1] * b;
        result[2] = a[2] * b;
    ...
    /// </summary>
    public static PGA3D Plane(Number a, Number b, Number c, Number d)
        => a * e1 + b * e2 + c * e3 + d * e0;
    ...
    /// </summary>
    public static PGA3D Point(Number x, Number y, Number z)
        => e123 + x * e032 + y * e013 + z * e021;

    /// <summary>
    /// Rotors (Euclidean lines)
    /// </summary>
    public static PGA3D Rotor(Number angle, PGA3D line)
        => (Number)Math.Cos(angle / 2.0f) + (Number)Math.Sin(angle / 2.0f) * line.Normalized();

    /// <summary>
    /// Translators (ideal lines)
    /// </summary>
    public static PGA3D Translator(Number distance, PGA3D line)
    ...
    public static PGA3D Circle(Number t, Number radius, PGA3D line)
        => Rotor(t * 2.0f * (Number)Math.PI, line) * Translator(radius, e1 * e0);

    // a torus is now the product of two circles. 
    public static PGA3D Torus(Number s, Number t, Number r1, PGA3D l1, Number r2, PGA3D l2)
...
public static class Program {
    private static PGA3D PointOnTorus(Number s, Number t) {
    ...
    public static void Main() {
        ...
        var rot = PGA3D.Rotor((Number)Math.PI / 2.0f, PGA3D.e1 * PGA3D.e2);

...
namespace PathTracer.Pathtracing.SceneDescription.Primitives.AccelerationStructures {
    ...
    public class AABB : ISceneObject {
        ...
        public void AddRange(IEnumerable<IBoundable> primitives) {
            ...
            foreach (Shape primitive in primitives) {
                Vector3[] bounds = primitive.Bounds;
                MinBound = Vector3.ComponentMin(MinBound, bounds[0]);
                MaxBound = Vector3.ComponentMax(MaxBound, bounds[1]);
            }
        ...
        public (Shape primitive, float distance)? IntersectPrimitives(Ray ray) {
            float closestDistance = ray.Length;
            Shape? closestPrimitive = null;
            foreach (Shape primitive in SceneObjects) {
